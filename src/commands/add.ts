...
import { MemTypeEnum, MemStateEnum, ConfidenceEnum } from '../types/schemas.js';
...
export const addCommand = new Command('add')
  ...
  .option('--reality-check', 'Include in reality-check output')
...
Examples:
  ...
  $ hypermem add "W = width, H = height" -t Lexicon --tags "shorthand"
  ...
  .action(async (statement: string, options) => {
    try {
      ...
      const result = await writePipeline({
        ...
        reality_check: options.realityCheck ?? false,
      });
